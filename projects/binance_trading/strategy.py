def evaluate_market(indicators: dict, balance: dict, rules: dict, current_price: float) -> dict:
    ...
    macd_data = indicators.get("MACD", {"macd": 0.0, "signal": 0.0, "histogram": 0.0})
    ...
    # 2. Baseline logic with MACD confirmation
    if rsi < 30 and macd > signal:  # MACD Confirmation for Buy
        action = "BUY"
        ...
        if investment > min_notional:
            ...
            reason = f"RSI undersold: {rsi:.2f}, MACD bullish"
